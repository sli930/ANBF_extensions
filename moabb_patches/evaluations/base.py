...
def _evaluate_fold(
    ...
    for group_idx, group_y, group_session in score_groups:
        ...
        try:
            ...
            y_pred = cvclf.predict(X[group_idx])
            predictions_folder_path = current_cache_data_path / f"{hdf5_path.parent.name}_predictions" / pipeline_name
            predictions_folder_path.mkdir(parents=True, exist_ok=True)
            save_predictions_path = predictions_folder_path / f"{hdf5_path.name}.txt"
            if save_predictions_path is not None:
                with open(save_predictions_path, 'a') as f:
                    for true, pred in zip(group_y, y_pred):
                        f.write(f"{true}\t{pred}\n")
